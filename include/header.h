    ...
    /*int cscanf(char *format, ...)
{
   ...
   return n;
    ...
    // Borra línea en la ventana de texto.
    // Delline borra la línea que contiene el cursor y mueve todas las líneas debajo
    // Es una alineación. Delline opera dentro de la ventana de texto actualmente activa.
    void delline()
    {
        COORD coord;
        SMALL_RECT origen;
    ...
    // Obtiene caracteres del teclado, no hace eco a la pantalla.
    // getch lee un solo carácter directamente desde el teclado, sin
    // haciendo eco a la pantalla.
    int getch(void)
    {
        int car;
        DWORD leidos, modo;
    ...
    // Copia texto de la pantalla de modo de texto a la memoria.
    // gettext almacena el contenido de un rectángulo de texto en pantalla definido por la izquierda,
    // Arriba, derecha y abajo en el área de memoria señalada por destino.
    // Todas las coordenadas son coordenadas de pantalla absolutas no relativas a la ventana. los
    // La esquina superior izquierda es (1,1). gettext lee el contenido del rectángulo
    // en memoria secuencialmente de izquierda a derecha y de arriba a abajo.
    // Cada posición en pantalla toma 2 bytes de memoria: el primer byte es el
    // El carácter en la celda y el segundo es el atributo de video de la celda. los
    ...
    int kbhit(void)
    {
        ...
        for (i = 0; !retval && i < nEventos; i++)
            if (eventos[i].EventType == KEY_EVENT)
                retval = 1;
        // delete[] eventos;
        free(eventos);
        return retval;
    ...
    // Define la ventana de modo de texto activo.
    // ventana define una ventana de texto en pantalla. Si las coordenadas son de alguna manera.
    // No válido, se ignora la llamada a la ventana.
    // izquierda y arriba son las coordenadas de la pantalla de la esquina superior izquierda de la
    // ventana.
    // derecha e inferior son las coordenadas de la pantalla de la esquina inferior derecha.
    // El tamaño mínimo de la ventana de texto es una columna por una línea. El valor por defecto
    // La ventana es pantalla completa, con las coordenadas:
    //    1,1, C, R
    // donde C es el número de columnas en el modo de video actual, y R es el
    // número de filas.
    void window(int left, int top, int right, int bottom)
    {
        vActual.left = left - 1;
        vActual.top = top - 1;
        vActual.right = right - 1;
        vActual.bottom = bottom - 1;
